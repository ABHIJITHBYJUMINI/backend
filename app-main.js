...
async function main(){
    createfyersobj(appid, secret, authcode).then( async fyers=>{
        const price = 120;                        // Example price
        const acceptedProfit = 0.5;                 // Profit percentage, e.g., 10%
        ...
        try {
            ...
            Order_Information = await updateOrderStatus(fyers, Order_Information );
            //update oldOders and profit for next trade also stop trading
            
            console.log("Order Information:", Order_Information);
            //check and act
